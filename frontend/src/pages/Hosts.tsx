      ...
      },
      {
        ...
        renderCell: (params) => (
          <IconButton
            aria-label="Delete"
            size="small"
            onClick={() => onDelete(params.row as Host)}
          >
      ...
      <Card>
        <CardContent>
          <Stack spacing={2}>
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
              <TextField label="Template" value={template} onChange={(e) => setTemplate(e.target.value)} fullWidth />
            </Stack>
            <Box>
      ...
      <Card>
        <CardContent>
          <Stack spacing={2}>
            ...
            <Typography color="text.secondary" variant="body2">
              Upload a `.csv` or `.xlsx` with columns: `hostname` (or `host`), `ip` (or `ip_address`), optional `template`.
            </Typography>
            <Box
              onDragOver={(e) => {
                e.preventDefault()
            ...
            >
              ...
              <Button variant="contained" onClick={onBulkUpload} disabled={!uploadFile || uploading}>
