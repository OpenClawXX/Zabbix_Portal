...
def create_token(user_id: int, username: str, roles: list[str], team_id: int | None) -> str:
    expire = datetime.now(timezone.utc) + timedelta(hours=_HOURS)
    return jwt.encode(
        {"sub": str(user_id), "username": username, "roles": roles, "team_id": team_id, "exp": expire},
        _SECRET,
        algorithm=_ALG,
    )
...
def require_admin(user: dict = Depends(get_current_user)) -> dict:
    ...
    if not any(r in roles for r in ("root", "team_lead")):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Team lead access required.")
    return user
